...
fprintf(filID,"function [coeff_deriv_fun,RHScoeffderiv] = get_coeff_deriv_matrices(derivative_orders,nmat)");
...
for ii=1:nmat
    for jj=1:size(matrix_tuples,1)
        ...
        fprintf(filID,strcat("coeff_deriv_fun{",index,"} = @(",output_variables{ii},") ",char(cross_deriv),";\n\n"));
    end
end
...
for jj=1:size(matrix_tuples,1)
    ...
    fprintf(filID,strcat("RHScoeffderiv{",index,"} = @(",output_variables{end},") ",vectorize(char(cross_deriv)),";\n\n"));
end
fprintf(filID,"end");
fclose(filID);
end
