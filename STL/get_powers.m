...
coeff_deriv = FEmatrices.RHScoeffderiv_fun{1,1,1};
parfor ii=1:param.nfreq
    v = zeros(1,param.ntheta);
    for jj=1:param.ntheta
        Pressure_surface = coeff_deriv(param.freq(ii),...
                                       param.theta(jj),...
                                       FEmatrices.Nodes(FEmatrices.PlateIn,FEmatrices.plan2D(1)),...
                                       FEmatrices.Nodes(FEmatrices.PlateIn,FEmatrices.plan2D(2)));
        v(jj) = 1i*2*pi*param.freq(ii)*Uortho(FEmatrices.PlateIn,ii,jj)'*...
